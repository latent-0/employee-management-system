...
const PhotoCapture: React.FC<PhotoCaptureProps> = ({ onPhotoVerified, onBack }) => {
    ...
    useEffect(() => {
        const startCamera = async () => {
            try {
                ...
                setError("Camera access is required to set a profile picture. Please enable it in your browser settings.");
            }
        };
        startCamera();
    ...
    const handleVerify = async (dataUrl: string) => {
        ...
        try {
            ...
            const result = await geminiService.verifyProfilePicture(base64, 'image/jpeg');
    ...
    return (
        <div className="space-y-4 text-center">
            <div className="relative w-full aspect-square bg-slate-200 dark:bg-slate-800 rounded-lg overflow-hidden flex items-center justify-center">
                {error && !isVerifying && <p className="text-sm text-red-500 p-4">{error}</p>}
